...
firebase.database().ref('/server').on('value', server => {
    firebase.database().ref('/streamOpen').on('value', payload => {
        ...
        if (streamOpen) {
            webcamExec = exec('ffmpeg -r 25 -f video4linux2 -i /dev/video0 -f mpegts -codec:v mpeg1video -s 640x480 ' + server.val().ip + ':' + server.val().streamPort + '/' + streamSecret + ' > /home/pi/Projects/interactive-board/webcam_device/webcam.log', (error, stdout, stderr) => {
                if (error === null) {
                    console.log('ok');
                } else {
                    console.log('exec error: ' + error);
